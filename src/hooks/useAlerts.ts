    ...
    const interval = setInterval(() => {
      ...
      for (const group of groups) {
        for (const buff of [...group.buffs, ...group.children]) {
          if (!isRuntimeBuff(buff) || !buff?.alert || !buff.alert.condition) continue;
...
function checkCondition(
  ...
  switch (condition) {
    ...
    case 'onCooldownEnd':
      return typeof buff.cooldownStart === 'number' && now - buff.cooldownStart < 1000;

    case 'onActive':
      return buff.status === 'Active' && now - (buff.statusChangedAt ?? 0) < 2000;

    case 'onInactive':
      return buff.status === 'Inactive' && now - (buff.statusChangedAt ?? 0) < 2000;
