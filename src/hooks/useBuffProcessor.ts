...
export function useBuffProcessor() {
    ...
    function processNormalBuff(storeBuff: BuffInstance, foundPayload: BuffInstance, now: number, finalPayloadMap: Map<string, any>) {
        const { name , timeRemaining: currentTimeRemaining, status: currentStatus } = storeBuff;

        const newTimeRemaining = foundPayload?.timeRemaining ?? 0;
        const shouldBeActive = newTimeRemaining > 0;
        const newStatus = shouldBeActive ? 'Active' : 'Inactive';
        if (newTimeRemaining !== currentTimeRemaining || newStatus !== currentStatus) {
            finalPayloadMap.set(name, {
                name,
                type: storeBuff.type,
                status: newStatus,
